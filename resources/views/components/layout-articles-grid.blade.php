...
<div class="grid grid-cols-3 gap-3 border-b border-gray-100 mb-3">
    ...
    @foreach($articles as $article)
        <a href="{{$article->link()}}" class="!text-gray-200 hover:!text-gray-200 no-underline">
            <div id="{{$article->hex}}" onmouseover="gridMouseOver(this.id)" onmouseout="gridMouseOut(this.id)" class="w-full h-44 bg-no-repeat bg-cover bg-center flex flex-col justify-end overflow-hidden" style="background-image: url('{{$article->getImageThumbnail()}}');">
